...
class AniNet(nn.Module):
    ...
    def forward(self, syn_input_ids, char_input_ids, syn_attention_mask, char_attention_mask, img_input_ids):
        ...
        output = self.img_classifier(img_output)
...
for epoch in tqdm(range(epochs), desc="Epoch"):
    ...
    #reporte.plot(f".//img_reportes//predVSreal_{epoch+1}.png")
    #reps = reporte.display()
    #print(reps)
    #reps.to_csv(".//img_reportes//reporte_{epoch+1}.csv")
...
#jsonfile = open(".//img_reportes//train_val_loss.json", "w")
#json.dump(trainval, jsonfile)
#jsonfile.close()

print("IMAGES SUCCESSFULLY RUNNED :)")
...
#anime_test["unscaled"] = valid_labels
anime_test["preds"] = predicciones
anime_test.to_csv(".//preds//preds_img.csv")
