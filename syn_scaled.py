...
anime_test = pd.read_csv(".//Database//Anime_test.csv")#.head(50)
...
#            self.img.append(retratos)
    ...
    def __getitem__(self, item):
        return {'synopsis':self.syn[item], 'mchar':self.mchars[item], 'img':[], 'label':self.labels[item]}
...
class AniNet(nn.Module):
    ...
    def forward(self, syn_input_ids, char_input_ids, syn_attention_mask, char_attention_mask, img_input_ids):
        ...
        #ALL:
        #concatenated_vectors = torch.concat((logits_a, logits_b), dim=1)
...
for epoch in tqdm(range(epochs), desc="Epoch"):
    ...
    #reporte.plot(f".//syn_reportes//predVSreal_{epoch+1}.png")
    #reps = reporte.display()
    #print(reps)
    #reps.to_csv(f".//syn_reportes//reporte_{epoch+1}.csv")
...
#jsonfile = open(".//syn_reportes//train_val_loss.json", "w")
...
anime_test.to_csv(".//preds//preds_syn.csv")
